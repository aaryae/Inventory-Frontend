...
const Inventory = () => {
  ...
  const filteredInventory = inventory.filter(item => {
    ...
    const matchesCategory = selectedCategory === 'all' || item.resourceType?.toLowerCase() === selectedCategory.toLowerCase();
    const matchesStatus = selectedStatus === 'all' || item.resourceStatus?.toLowerCase() === selectedStatus.toLowerCase();
  ...
  if (loading) {
    return (
      <div className="p-6 flex items-center justify-center h-64 text-slate-400">Loading resources...</div>
    );
  }
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:space-x-4 space-y-4 lg:space-y-0">
          ...
          <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
            ...
            >
              ...
              {/* You can dynamically generate these from your data if needed */}
          ...
          <table className="w-full">
            <thead style={{ background: "#21222d" }}>
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">Specification</th>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">Purchase Date</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">Warranty Expiry</th>
            ...
            <tbody className="divide-y divide-[#21222d]">
              {filteredInventory.length === 0 ? (
                ...
                filteredInventory.map((item) => (
                  <tr key={item.resourceId} className="hover:bg-[#21222d] transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">{item.resourceCode}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{item.brand}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{item.model}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{item.specification}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{item.resourceType}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{item.resourceClass}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(item.resourceStatus)}`}>
                        {item.resourceStatus}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">{item.purchaseDate}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{item.warrantyExpiry}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex items-center justify-end space-x-2">
                        <button onClick={() => handleViewResource(item.resourceId)} className="text-cyan-400 hover:text-cyan-300 p-1">
                          <Eye className="w-4 h-4" />
                        </button>
                        <button className="text-yellow-400 hover:text-yellow-300 p-1">
                          <Pencil className="w-4 h-4" />
                        </button>
                        <button onClick={() => handleDeleteResource(item.resourceId)} className="text-red-400 hover:text-red-300 p-1">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-[#171821] rounded-2xl p-6 w-full max-w-md mx-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold text-slate-100">Resource Details</h3>
              <button
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-slate-100 font-medium">{selectedResource.resourceCode}</p>
              </div>
              <div>
                <label className="text-sm text-slate-400">Brand</label>
                <p className="text-slate-100 font-medium">{selectedResource.brand}</p>
              </div>
              <div>
                <label className="text-sm text-slate-400">Model</label>
                <p className="text-slate-100 font-medium">{selectedResource.model}</p>
              </div>
              <div>
                <label className="text-sm text-slate-400">Specification</label>
                <p className="text-slate-100 font-medium">{selectedResource.specification}</p>
              </div>
              <div>
                <label className="text-sm text-slate-400">Type</label>
                <p className="text-slate-100 font-medium">{selectedResource.resourceType}</p>
              </div>
              <div>
                <label className="text-sm text-slate-400">Class</label>
                <p className="text-slate-100 font-medium">{selectedResource.resourceClass}</p>
              </div>
              <div>
                <label className="text-sm text-slate-400">Status</label>
                <p className="text-slate-100 font-medium">{selectedResource.resourceStatus}</p>
              </div>
              <div>
                <label className="text-sm text-slate-400">Purchase Date</label>
                <p className="text-slate-100 font-medium">{selectedResource.purchaseDate}</p>
              </div>
              <div>
                <label className="text-sm text-slate-400">Warranty Expiry</label>
                <p className="text-slate-100 font-medium">{selectedResource.warrantyExpiry}</p>
              </div>
              <div>
                <label className="text-sm text-slate-400">Batch Code</label>
                <p className="text-slate-100 font-medium">{selectedResource.batchCode}</p>
              </div>
            </div>
