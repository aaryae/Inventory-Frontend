import { Boxes, CheckCircle2, CheckCircle, Backpack, ScanLine, Home, Users, X } from "lucide-react";
...
const Usersidebar = ({ isOpen, toggleSidebar }) => {
  ...
  const menuItems = [
    ...
    { name: "Inventory", icon: Boxes, path: "/user/inventory" },
    { name: "Master", icon: CheckCircle, path: "/user/master" },
    { name: "Batch", icon: Backpack, path: "/user/batch" },
    { name: "Barcode", icon: ScanLine, path: "/user/barcode" },
  ];
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 left-0 h-full w-64 text-slate-100 shadow-2xl shadow-black/40 border-r border-[#21222d] z-50
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5 border-t border-[#21222d] bg-gradient-to-t from-[#171821] to-transparent">
          <div className="flex items-center space-x-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-m text-slate-400 truncate">{name}</p>
            </div>
          </div>
        </div>
      </div>
